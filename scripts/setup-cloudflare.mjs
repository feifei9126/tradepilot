...
const NEXT_FIX = {
  ...
  "secret TRADEPILOT_CREDENTIALS_KEY": "Set a valid 32-byte base64url credential key in Wrangler and retry",
  "secret TRADEPILOT_CRON_SECRET": "Set the scheduled outbox Bearer secret in Wrangler and retry",
...
function generatedBase64UrlSecret() {
  return crypto.randomBytes(32).toString("base64url");
}

const RUNTIME_SECRET_KEYS = ["AUTH_SECRET", "TRADEPILOT_CREDENTIALS_KEY", "TRADEPILOT_CRON_SECRET"];

async function readRuntimeSecrets(path) {
  let source;
  try {
  ...
  for (const line of source.split(/\r?\n/)) {
    ...
    if (RUNTIME_SECRET_KEYS.includes(key)) values[key] = trimmed.slice(separator + 1).trim();
  }
  return values;
}

async function writeRuntimeSecrets(path, values) {
  const contents = [
    "# Generated by npm run setup:cloudflare. Keep this file private and backed up.",
    ...RUNTIME_SECRET_KEYS.map((key) => `${key}=${values[key]}`),
    "",
  ].join("\n");
...
function validateCredentialsKey(value) {
  const key = String(value || "").trim();
  if (!/^[A-Za-z0-9_-]{43}$/.test(key) || Buffer.from(key, "base64url").length !== 32) {
    throw new Error("TRADEPILOT_CREDENTIALS_KEY must be a 32-byte base64url value");
  }
  return key;
}

function validateCronSecret(value) {
  const secret = String(value || "").trim();
  if (!/^[A-Za-z0-9_-]{43,128}$/.test(secret)) {
    throw new Error("TRADEPILOT_CRON_SECRET must be a secure base64url value");
  }
  return secret;
}
...
export async function runCloudflareSetup({
  ...
  const stateFile = resolve(cwd, String(env.TRADEPILOT_CLOUDFLARE_STATE_FILE || ".env.cloudflare"));

  try {
    const storedSecrets = dryRun ? {} : await readRuntimeSecrets(stateFile);
    ...
    const authSecret = String(env.AUTH_SECRET || storedSecrets.AUTH_SECRET || "").trim() || generatedAuthSecret();
    const credentialsKey = validateCredentialsKey(env.TRADEPILOT_CREDENTIALS_KEY || storedSecrets.TRADEPILOT_CREDENTIALS_KEY || generatedBase64UrlSecret());
    const cronSecret = validateCronSecret(env.TRADEPILOT_CRON_SECRET || storedSecrets.TRADEPILOT_CRON_SECRET || generatedBase64UrlSecret());
    secrets.add(databaseUrl);
    secrets.add(adminPassword);
    secrets.add(authSecret);
    secrets.add(credentialsKey);
    secrets.add(cronSecret);
    if (!dryRun) {
      await writeRuntimeSecrets(stateFile, { AUTH_SECRET: authSecret, TRADEPILOT_CREDENTIALS_KEY: credentialsKey, TRADEPILOT_CRON_SECRET: cronSecret });
      log(`Cloudflare runtime secrets saved to ${stateFile}`);
    }
    ...
    await runStep("secret TRADEPILOT_CREDENTIALS_KEY", "npx", ["wrangler", "secret", "put", "TRADEPILOT_CREDENTIALS_KEY"], {}, credentialsKey);
    await runStep("secret TRADEPILOT_CRON_SECRET", "npx", ["wrangler", "secret", "put", "TRADEPILOT_CRON_SECRET"], {}, cronSecret);
