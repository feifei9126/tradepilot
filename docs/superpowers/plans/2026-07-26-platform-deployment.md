...
入口把 `fetch(request, env, ctx)` 转发给 `openNextWorker.fetch(request, env, ctx)`；在导入构建产物的那一行使用 `// @ts-expect-error: generated by npm run cfbuild`，只抑制 `tsc` 对不存在于源码树的 `.open-next/worker.js` 报错；`scheduled` 调用 Resend outbox processor；`email` 将 Cloudflare raw message 交给 inbound parser。处理器错误写结构化日志并返回，不把邮件正文/凭据写日志。
