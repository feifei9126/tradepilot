        ...
        {field.type === "textarea" ? (
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              onChange={(e) => onChange(field.key, e.target.value)}
              className="w-10 h-9 p-0.5 border rounded"
              disabled={disabled}
            />
            <Input
              value={String(scalarValue)}
...
function MultiSelect({
  ...
  const unselected = (field.options || []).filter(
    (o: FieldOption) => !selected.includes(o.value),
  );
  const add = (v: string | null) => v && onChange([...selected, v]);
  const remove = (v: string) =>
    onChange(selected.filter((s: string) => s !== v));
  return (
    <div className="space-y-1.5">
      <div className="flex flex-wrap gap-1.5">
        {selected.map((v: string) => {
          const opt = (field.options || []).find(
            (o: FieldOption) => o.value === v,
          );
          return (
...
export function FieldBuilder({
  ...
  const upd = (id: string, u: Partial<CustomFieldDef>) =>
    onChange(fields.map((f) => (f.id === id ? { ...f, ...u } : f)));
          ...
          <div key={f.id} className="border rounded-lg p-4 bg-white space-y-3">
            <div className="flex items-center justify-between">
              ...
              <Button
                ...
                className="h-6 w-6"
                onClick={() => del(f.id)}
                aria-label={`删除字段 ${f.name || `#${i + 1}`}`}
                title={`删除字段 ${f.name || `#${i + 1}`}`}
              >
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div className="space-y-1">
                ...
                <Input
                  value={f.key}
                  onChange={(e) => upd(f.id, { key: e.target.value })}
                  placeholder="customerGrade"
              ...
              <div className="space-y-1">
                ...
                <Select
                  value={f.type}
                  onValueChange={(v) =>
                    v && upd(f.id, { type: v as FieldType })
                  }
                >
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="space-y-1">
                ...
                <Input
                  value={f.placeholder || ""}
                  onChange={(e) => upd(f.id, { placeholder: e.target.value })}
                  className="h-8 text-sm"
                />
              </div>
              <div className="space-y-1">
                ...
                <Input
                  ...
                  onChange={(e) => upd(f.id, { defaultValue: e.target.value })}
                  className="h-8 text-sm"
                />
              </div>
              <div className="space-y-1">
                ...
                <Select
                  ...
                  onValueChange={(v) => upd(f.id, { section: v || undefined })}
                >
                  <SelectTrigger className="h-8 text-sm">
            ...
            <div className="flex items-center gap-4 text-xs">
              <label className="flex items-center gap-1.5">
                <input
                  type="checkbox"
                  checked={!!f.required}
                  onChange={(e) => upd(f.id, { required: e.target.checked })}
                />
                必填
              </label>
              <label className="flex items-center gap-1.5">
                <input
                  ...
                  onChange={(e) =>
                    upd(f.id, { width: e.target.checked ? "half" : undefined })
                  }
                />
                半宽布局
              </label>
